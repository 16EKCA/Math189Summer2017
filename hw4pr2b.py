...
def NLL(X, y, W, reg=0.0):
	...
	NLL= -(y*np.log(exp_mu/exp_mu.sum(axis=1).reshape(-1,1))).sum(axis=1).sum() +reg*np.diag(W.T @ W).sum()
	"*** END YOUR CODE HERE ***"
	return NLL
...
def grad_softmax(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	grad=X.T @ (exp_mu/exp_mu.sum(axis=1).reshape(-1,1)-y)+reg*W
	"*** END YOUR CODE HERE ***"
	return grad
...
def predict(X, W):
	...
	# Weight given
	"*** YOUR CODE HERE ***"
	y_pred=np.argmax(exp_mu/exp_mu.sum(axis=1).reshape(-1, 1), axis=1).reshape(-1, 1)
	"*** END YOUR CODE HERE ***"
	return y_pred
