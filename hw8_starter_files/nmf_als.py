...
def nmf_cost(X, W, H):
	...
	for index in range(len(sparseX.data)):
		row=sparseX.row[index]
		col=sparseX.col[index]
		data=sparseX.data[index]
		cost=cost+(data-(W[row,:] @ H[:,col]).item(0))**2
	"*** END YOUR CODE HERE ***"
	return cost
...
def nmf(X, k=20, max_iter=100, print_freq=5):
	...
	for iter_num in range(max_iter):
		...
		cost_list.append(nmf_cost(X, W, H))
		"*** END YOUR CODE HERE ***"
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	indexOFfequentWords=np.flip(np.argsort(H, axis=1), 1)
	"*** END YOUR CODE HERE ***"
	top_words = np.array(tfidf.get_feature_names())[indexOFfequentWords]
