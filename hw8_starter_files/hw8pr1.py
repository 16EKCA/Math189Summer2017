...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for clusterCenter in range(k):
			ind=np.where(label == clusterCenter)[0]
			if len(ind) > 0:
				clusters[clusterCenter, :] = X[ind].mean(axis=0)
		...
		if np.linalg.norm(copy.deepcopy(clusters) - clusters) <= eps:
...
def k_means_cost(X, clusters, label):
	...
	"*** YOUR CODE HERE ***"
	Xcluster=clusters[label.flatten()]

	cost=(np.linalg.norm(X-Xcluster, axis=1)**2).sum()
	"*** END YOUR CODE HERE ***"
	return cost
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	CostVsKplot,=plt.plot(range(1, 21),cost_k_list, 'g^')
	
	optCostPlot,=plt.plot(opt_k,min(cost_k_list), 'rD')
	"*** END YOUR CODE HERE ***"
	...
	dataPlot,=plt.plot(X[:, 0], X[:, 1], 'bo')

	centerPlot,=plt.plot(Xcluster[:, 0], Xcluster[:, 1], 'rD')
	"*** END YOUR CODE HERE ***"

	# set up legend and save the plot to the current folder
	plt.legend((dataPlot, centerPlot), \
