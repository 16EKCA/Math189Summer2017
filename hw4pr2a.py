...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	nll = -sum(np.multiply(y,np.log(sigmoid(X @ W)))+np.multiply((1.-y),np.log(1.-sigmoid(X @ W))))+reg/2*np.linalg.norm(W)**2
...
def grad_descent(X, y, reg=0.0, lr=1e-4, eps=1e-6, max_iter=500, print_freq=20):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		if np.isnan(NLL(X, y, W, reg=reg)):
			break
		nll_list.append(NLL(X, y, W, reg=reg))

		W_grad=grad_logreg(X, y, W, reg=0.0)
...
def newton_step(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = sigmoid(X @ W)
	d = linalg.solve((X.T @ diag @ X+reg*np.eye(X.shape[1])),(grad_logreg(X,y,W,reg=reg)))
	"*** END YOUR CODE HERE ***"
	return d
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
	...
	while iter_num < max_iter and np.linalg.norm(step) > eps:
		...
		"*** YOUR CODE HERE ***"
		iteration = newton_step(X, y, W, reg=reg)
		W = W - iteration
		nll_list.append(log_likelihood(X, y, W, reg=reg))
